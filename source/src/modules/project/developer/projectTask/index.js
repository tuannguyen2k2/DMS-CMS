...
import LeaderDetailProjectTaskModal from './DetailProjectTaskModal';
import DeveloperDetailProjectTaskModal from './DetailProjectTaskModal';
...
function DeveloperProjectTaskListPage({ setSearchFilter }) {
    ...
    const { data, mixinFuncs, queryFilter, loading, pagination, changePagination, queryParams, serializeParams } =
        useListBase({
            ...
            override: (funcs) => {
                ...
                funcs.getCreateLink = () => {
                    return `${routes.developerProjectTaskListPage.path}/create?projectId=${projectId}&projectName=${projectName}&active=${active}`;
                };
                funcs.getItemDetailLink = (dataRow) => {
                    return `${routes.developerProjectTaskListPage.path}/${dataRow.id}?projectId=${projectId}&projectName=${projectName}&active=${active}`;
                };
                funcs.additionalActionColumnButtons = () => ({
                    taskLog: ({ id, taskName, developer }) => {
                        if (developer?.id == profile?.id)
                            return (
                    ...
                    state: (item) => {
                        if (item?.developer?.id == profile?.id)
                            return (
    ...
    const columns = [
        ...
        active && mixinFuncs.renderActionColumn({ taskLog: true, state: true }, { width: '180px' }),
    ].filter(Boolean);
    ...
    return (
        <div>
            ...
            <DeveloperDetailProjectTaskModal
                open={openedModal}
...
export default DeveloperProjectTaskListPage;
