    ...
    const { detail, mixinFuncs, loading, onSave, setIsChangedFormValues, isEditing, title } = useSaveBase({
        ...
        override: (funcs) => {
            ...
            funcs.onSaveError = (err) => {
                if (err.code === 'ERROR-USER-0000') {
                    console.log(1);
                    showErrorMessage(translate.formatMessage(commonMessage.error),translate.formatMessage(commonMessage.existPhone));
                } else if (err.code === 'ERROR-DB-QUERY-0000') {
                    showErrorMessage(translate.formatMessage(commonMessage.error),translate.formatMessage(commonMessage.existEmail));
                } else {
