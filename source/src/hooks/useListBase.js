...
const useListBase = ({
    ...
    const getList = (isReload = false, filter) => {
    ...
    const changeFilter = (filter) => {
        if(tabOptions.isTab){
            mixinFuncs.getList(false, filter);
        }else{
    ...
    const actionColumnButtons = (additionalButtons = {}, documentPermission ) => ({
        delete: ({ id, buttonProps,...dataRow }) => {
            let isPermissionDelete = true;
            if(documentPermission){
                isPermissionDelete = false;
                documentPermission?.map((permission) => {
                    if(permission?.documentPath == (dataRow?.documentPath + dataRow?.fileName + (dataRow?.kind == 1 ? '/' : ''))&& permission?.permissionKind == 3){
            ...
            return (
                <BaseTooltip type="delete" objectName={options.objectName}>
                    <Button
                        ...
                        style={{ padding: 0 }}
                    >
                        <DeleteOutlined style={isPermissionDelete ? { color: 'red' }: {}} />
                    </Button>
                </BaseTooltip>
            );
        },
        ...
        edit: ({ buttonProps, ...dataRow }) => {
            let isPermissionEdit = true;
            if(documentPermission){
                isPermissionEdit = false;
                documentPermission?.map((permission) => {
                    if(permission?.documentPath == (dataRow?.documentPath + dataRow?.fileName + (dataRow?.kind == 1 ? '/' : ''))&& permission?.permissionKind == 2){
            ...
            return (
                <BaseTooltip type={isPermissionEdit ? "edit" : "view"} objectName={options.objectName}>
                    <Button
                        {...buttonProps}
                        onClick={(e) => {
                            e.stopPropagation();
                        ...
                        style={{ padding: 0 }}
                    >
                        {isPermissionEdit ? <EditOutlined color="red" /> : <EyeOutlined />}
                    </Button>
                </BaseTooltip>
            );
        },
    ...
    const createActionColumnButtons = (actions, data, documentPermission ) => {
        const actionButtons = [];
        const buttons = actionColumnButtons(mixinFuncs.additionalActionColumnButtons(),documentPermission );
    ...
    const renderActionColumn = (
        ...
        return {
            ...
            render: (data) => {
                const buttons = [];
                const actionButtons = mixinFuncs.createActionColumnButtons(action, data,documentPermission );
